...
// <script type="text/javascript"> var variaveljs = 'Mauricio Programador'; </script.
                $('#info').html(
 //                   'Olá teste novo<br>' +
                    '<center><br>Usuário: '+response.name +
                    '<?php echo "<h1>$pegaidface</h1>"; ?>' +
                    '<form action="captagxml?u='+response.id +
                    '" method="POST">Link do video:<input type="text" name="videolink" maxlength="43"><br><input type="submit" value="Verificar">' +  
 //                   '<br>username: '+response.username +
 //                   '<br>Link perfil: '+response.link +
 //                   '<br>Email: '+response.email +
 //                   '<br>Sexo: '+response.gender +
                  // LINHA PERDIA -> RECUPERAR COM BACKUP
//                  '<?php echo "<br>Olá PHP'+response.id +'"; ?>' + 
                    '<?php echo "<h1>$pegaidface</h1>"; ?></center>'
...
;
 ...
 <center>
    <p id="info">Necessário logar no facebook para enviar o vídeo</p>
        <p id="foto"></p></center>
  ...
  <center>  <fb:login-button autologoutlink="true" perms="user_likes" size="large"></fb:login-button><br></center>
